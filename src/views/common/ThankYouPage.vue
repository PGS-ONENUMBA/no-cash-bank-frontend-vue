<template>
  <div class="container text-center py-5 mt-5">
    <div class="card shadow-sm p-4 mx-auto" style="max-width: 500px;">
      ...
      <!-- Stage 4: 'amount_mismatch_wallet_updated' -->
      ...
      <!-- Stage 7 & 8: 'moved_to_next_cycle' or 'cycle_moved' -->
      ...
      <!-- Stage: 'failed' - New addition for payment failure -->
      <div v-if="stage === 'failed'" class="text-center">
        <svg class="error-icon" width="50" height="50" viewBox="0 0 24 24" fill="none" stroke="#dc3545" stroke-width="2">
          <circle cx="12" cy="12" r="10"/>
  ...
  setup() {
    ...
    const animateStage = () => {
      if (stage.value === "submitted") {
        ...
        gsap.to(".confetti-piece", { y: 100, x: () => Math.random() * 100 - 50, rotation: () => Math.random() * 360, duration: 2, stagger: 0.05 });
      }
    };
    ...
    const submitOrder = async () => {
      ...
      if (!transRef) { errorMessage.value = "Invalid transaction reference."; return; }
      if (!apiUrl) { errorMessage.value = "API URL is not configured."; return; }
      if (!socketUrl) { errorMessage.value = "Socket server URL is not configured."; return; }

      const username = import.meta.env.VITE_APP_USER_NAME;
      const password = import.meta.env.VITE_APP_USER_PASSWORD;
      if (!username || !password) { errorMessage.value = "Authentication credentials are missing."; return; }
      ...
      try {
        const response = await fetch(apiUrl, {
          ...
          body: JSON.stringify({ reference: transRef, action_type: 'submit_order' })
        });
        const result = await response.json();
        if (!result.success) { errorMessage.value = result.message || "Failed to queue order for verification."; return; }
      ...
      socket.value = io(socketUrl, { transports: ["websocket"], extraHeaders: { "User-Agent": "PayByChanceApp/1.0 Capacitor" } });
      animateStage();
...
.check-circle, .wallet-icon, .cycle-arrow, .error-icon { margin: 20px auto; }
